...
- **LVGL 图形界面**：240×320 ST7789 RGB565 彩屏，文件列表浏览、曲目名滚动、播放状态、音量条、进度条与已播放/总时长显示
- **5 键实体操控**：播放/暂停、上一曲、下一曲、音量加、音量减；30 ms 软件消抖，音量键支持 500 ms 长按后每 200 ms 连发
...
| LCD | RST | 3 | |
| 按键 | 播放/暂停 | 10 | |
| 按键 | 上一曲 | 11 | 停止态为列表光标上移 |
| 按键 | 下一曲 | 12 | 停止态为列表光标下移 |
| 按键 | 音量 + | 13 | 长按连发 |
| 按键 | 音量 − | 14 | 长按连发 |
...
| `monitor_task` | Core 1 | 1 | 2 KB | 100 ms 周期刷新 UI，并根据播放状态显示/隐藏文件列表 |
...
2. 插卡上电，屏幕先亮并进入文件列表（列表为空且 `/music` 不存在时会自动尝试创建该目录）；
3. **停止状态**下：按【上一曲】/【下一曲】移动列表光标，按【播放】播放选中曲目；
4. **播放状态**下：按【播放】暂停/继续，按【上一曲】/【下一曲】切歌，按【音量 ±】调音量（按住连发）；
5. 单曲播放结束自动播放下一首，列表循环；
6. SD 卡不在位时系统照常启动，只是列表为空、按播放无动作。
